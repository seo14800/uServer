...
bool Room::EnterRoom(ObjectRef object, bool randPos /*= true*/)
{
	...
	if (randPos)
	{
		object->posInfo->set_x(Utils::GetRandom(0.f, 1000.f));
		object->posInfo->set_y(Utils::GetRandom(0.f, 1000.f));
...
{
	string chatMsg = pkt.msg();
	uint64 playerID = pkt.playerid();
	Protocol::S_CHAT sendPkt;

	sendPkt.set_playerid(playerID);
	sendPkt.set_msg(chatMsg);

	cout << "Broadcast chat pkt msg :  " << chatMsg << ", objID : " << playerID;
...
void Room::UpdateTick()
{
	...
	cout << "Current Players : " << _objects.size() << endl;
